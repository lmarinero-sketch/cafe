...
// System prompt defining Oliver's persona and rules
...
// Helper: Formatter for Argentine Pesos
export function formatARS(amount: number): string {
  return new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 2 }).format(amount);
}

// Tool execution logic interacting with Supabase
async function executeOliverTool(name: string, args: Record<string, any>): Promise<{ text: string, exportInfo?: ExcelExportInfo }> {
  try {
    switch (name) {
      case 'query_kpis': {
        const [ordersRes, custRes, prodRes, tablesRes, cashRes] = await Promise.all([
          ...
          supabase.from('products').select('id, is_available', { count: 'exact' }),
          supabase.from('tables').select('id, status'),
          supabase.from('cash_registers').select('id, status, initial_balance, created_at').eq('status', 'open').order('created_at', { ascending: false }).limit(1)
        ]);

        const orders = ordersRes.data || [];
        const totalSales = orders.reduce((sum, o) => sum + (Number(o.total) || 0), 0);
        const completedOrders = orders.filter(o => o.status === 'completed' || o.status === 'cobrado');
      ...
      case 'query_products': {
        let q = supabase.from('products').select('id, name, category_name, price, cost, suggested_price, is_available, description');
      ...
      case 'query_orders': {
        let q = supabase.from('orders').select('id, code, customer_name, total, status, type, payment_method, created_at, table_name');
        if (args.status) q = q.eq('status', args.status);
        if (args.type) q = q.eq('type', args.type);
        const { data, error } = await q.order('created_at', { ascending: false }).limit(args.limit || 15);
        if (error) return { text: JSON.stringify({ error: error.message }) };

        const totalMonto = (data || []).reduce((sum, o) => sum + (Number(o.total) || 0), 0);
      ...
      case 'query_customers': {
        let q = supabase.from('customers').select('id, first_name, last_name, phone, email, points, total_spent, purchase_count, level');
        if (args.search) {
      ...
      case 'query_tables': {
        let q = supabase.from('tables').select('id, number, capacity, sector, status');
        ...
        return {
          text: JSON.stringify({
            ...
            disponibles: tables.filter(t => t.status === 'disponible').length,
      ...
      case 'query_cash': {
        const { data: registers } = await supabase.from('cash_registers').select('*').order('created_at', { ascending: false }).limit(1);
        ...
        const ingresos = transactions.filter(t => t.type === 'ingreso').reduce((s, t) => s + (Number(t.amount) || 0), 0);
        const egresos = transactions.filter(t => t.type === 'egreso').reduce((s, t) => s + (Number(t.amount) || 0), 0);
        ...
        return {
          text: JSON.stringify({
            ...
            ultimos_movimientos: transactions.slice(0, 5).map(t => ({
              tipo: t.type,
      ...
      case 'query_ingredients': {
        let q = supabase.from('ingredients').select('*');
        if (args.search) q = q.ilike('name', `%${args.search}%`);
      ...
      case 'create_product': {
        const { data, error } = await supabase.from('products').insert({
          ...
          is_available: true,
          is_featured: false,
          channels: ['dine_in', 'delivery', 'take_away']
        }).select().single();
      ...
      case 'update_table_status': {
        const { data: table, error: findError } = await supabase.from('tables').select('id, number').eq('number', args.table_number).single();
        if (findError || !table) return { text: JSON.stringify({ error: `Mesa ${args.table_number} no encontrada` }) };

        const { error: updateError } = await supabase.from('tables').update({ status: args.new_status }).eq('id', table.id);
      ...
      case 'create_cash_transaction': {
        const { data: registers } = await supabase.from('cash_registers').select('id').eq('status', 'open').order('created_at', { ascending: false }).limit(1);
      ...
      case 'generate_excel': {
        ...
        // Perform instant export generation in browser
        const downloadSuccess = await generateAndTriggerExcel(repType, filename, exportTitle);
        const rowCount = downloadSuccess.rowCount;

        const tag = `[DESCARGAR_EXCEL:${repType}:${filename}:${exportTitle}]`;
        return {
          text: JSON.stringify({
            success: true,
            mensaje: `Excel de ${repType} generado con éxito (${rowCount} filas). Se inició la descarga automática en el navegador.`,
            tag_descarga: tag,
            filas_generadas: rowCount
          }),
          exportInfo: {
            type: repType,
            filename,
...
// Main function to invoke Oliver with multi-round OpenAI tool-calling loop
export async function askOliver(
  ...
  if (!OPENAI_API_KEY) {
    return {
      reply: '⚠️ La clave de API de OpenAI no está configurada en las variables de entorno (`VITE_OPENAI_API_KEY`). Por favor configúrala para conversar con Oliver.'
    };
  }

  const routeContext = `\n\n## CONTEXTO DE NAVEGACIÓN ACTUAL:
El usuario está visualizando la ruta: "${activeRoute}".
Ten en cuenta esta pantalla para ofrecer datos o sugerir acciones rápidas relevantes a esa área.`;
  ...
  try {
    ...
    while (rounds < 4) {
      ...
      if (assistantMsg.tool_calls && assistantMsg.tool_calls.length > 0) {
        for (const tc of assistantMsg.tool_calls) {
          ...
          const { text: toolResultText, exportInfo } = await executeOliverTool(fnName, fnArgs);
          
          if (exportInfo) {
            foundExportTag = `[DESCARGAR_EXCEL:${exportInfo.type}:${exportInfo.filename}:${exportInfo.title}]`;
          }
    ...
    console.error('Error in askOliver:', error);
    return {
      reply: `⚠️ Ocurrió un inconveniente al consultar con Oliver: ${error.message || 'Error de conexión'}`
    };
  }
}
